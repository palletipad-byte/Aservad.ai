...
# 4. AI అడ్వాన్స్‌డ్ ఇమేజ్ జనరేటర్ (Replicate Flux Model - Joshna Tailors & Aservad.ai)
elif choice.startswith("4."):
    st.subheader("🎨 AI అల్ట్రా-హెచ్‌డీ ఇమేజ్ జనరేటర్ (Joshna Tailors & Aservad.ai)")
    st.info("మిత్రమా, ఇక్కడ మీకు కావలసిన డిజైన్, ఫ్యాషన్ లుక్ లేదా ఫోటో వివరాలను రాయండి. Replicate Flux A.I. దాన్ని అచ్చం నిజమైన ఫోటోలా, అద్భుతమైన షైనింగ్‌తో సృష్టించి ఇస్తుంది!")

    user_img_prompt = st.text_area(
        "మీకు కావలసిన డిజైన్ లేదా ఇమేజ్ వివరాలు ఇక్కడ రాయండి:", 
        placeholder="ఉదాహరణకు: Beautiful modern bridal blouse design with heavy embroidery work, gold shine, 8k resolution, ultra-realistic...",
        key="advanced_img_input"
    )

    if st.button("🚀 ఇమేజ్ సృష్టించండి (Generate Image)", key="gen_img_btn"):
        if user_img_prompt.strip():
            with st.spinner("✨ అద్భుతమైన డిజైన్‌కు రూపం ఇస్తున్నాం... దయచేసి వేచి ఉండండి మిత్రమా!"):
                try:
                    import replicate
                    import os

                    # Streamlit secrets నుండి Replicate API Token సెట్ చేయడం
                    os.environ["REPLICATE_API_TOKEN"] = st.secrets["REPLICATE_API_TOKEN"]

                    # ప్రపంచంలోనే అత్యుత్తమమైన Flux ఇమేజ్ జెనరేషన్ మోడల్
                    output = replicate.run(
                        "black-forest-labs/flux-schnell",
                        input={
                            "prompt": user_img_prompt + ", ultra-detailed, photorealistic, 8k, stunning lighting, professional fashion photography",
                            "go_fast": True,
                            "num_outputs": 1,
                            "aspect_ratio": "1:1",
                            "output_format": "webp",
                    ...
                    if output:
                        ...
                        st.image(output[0], caption="Generated by Joshna & Aservad AI (Flux Model)", use_column_width=True)
                    else:

...
# 4. AI అడ్వాన్స్‌డ్ ఇమేజ్ జనరేటర్ (Google Gemini Official Engine)
elif choice.startswith("4."):
    st.subheader("🎨 AI అల్ట్రా-హెచ్‌డీ ఇమేజ్ జనరేటర్ (Joshna Tailors & Aservad.ai)")
    st.info("మిత్రమా, ఇక్కడ మీకు కావలసిన డిజైన్ లేదా ఫోటో వివరాలను రాయండి. ఇది గూగుల్ జెమినీ ఏఐ ద్వారా అసలైన రియలిస్టిక్ ఫోటోను సృష్టిస్తుంది!")
    
    user_img_prompt = st.text_area(
        "మీకు కావలసిన డిజైన్ లేదా ఇమేజ్ వివరాలు ఇక్కడ రాయండి:",
        placeholder="ఉదాహరణకు: A stunning, ultra-realistic photorealistic cinematic portrait of the famous South Indian actress Nayanthara..."
    )
    
    if st.button("🚀 ఇమేజ్ సృష్టించండి (Generate Image)", key="gen_img_btn"):
        if user_img_prompt.strip():
            with st.spinner("⏳ గూగుల్ జెమినీ ఏఐ సహాయంతో అద్భుతమైన రియల్ ఫోటోను సృష్టిస్తున్నాం... దయచేసి వేచి ఉండండి."):
                try:
                    from google import genai
                    from PIL import Image
                    from io import BytesIO
                    
                    # 🎯 గూగుల్ జెమినీ క్లయింట్ కనెక్షన్
                    client = genai.Client(api_key=st.secrets["GEMINI_API_KEY"])
                    
                    # 🎯 గూగుల్ సిఫార్సు చేసిన లేటెస్ట్ మోడల్ (gemini-2.5-flash-image)
                    response = client.models.generate_content(
                        model='gemini-2.5-flash-image',
                        contents=f"Generate a highly detailed, ultra-realistic photograph, 8k resolution, professional studio lighting: {user_img_prompt}"
                    )
                    
                    # 🖼️ రెస్పాన్స్ నుండి ఇమేజ్ డేటాను సేకరించడం
                    image_found = False
                    for part in response.candidates[0].content.parts:
                        if part.inline_data:
                            ...
                            st.image(image, caption="Generated by Google Gemini & Aservad.ai", use_container_width=True)
                            image_found = True
                            break
                    
                    if not image_found:
                        st.error("⚠️ సర్వర్ నుండి ఇమేజ్ డేటా అందలేదు, దయచేసి మరోసారి ప్రయత్నించండి మిత్రమా!")

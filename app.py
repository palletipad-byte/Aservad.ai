...
# 3. వాయిస్ క్లోనింగ్ / ఆడియో అసిస్టెంట్ (అడ్వాన్స్‌డ్)
    ...
    st.info("ముందుగా మీ వాయిస్ శాంపిల్ అప్‌లోడ్ చేయండి, ఆపై టెక్స్ట్ టైప్ చేయండి.")
    
    # ఆడియో అప్‌లోడ్
    audio_file = st.file_uploader("ఆడియో ఫైల్ అప్‌లోడ్ చేయండి (WAV/MP3):", type=["wav", "mp3"])
        ...
        if st.button("వాయిస్ క్లోనింగ్ ప్రారంభించండి", key="voice_btn"):
            if text_to_speak:
                with st.spinner('క్లోనింగ్ జరుగుతోంది... దయచేసి వేచి ఉండండి...'):
                    ...
                    st.balloons()
                    st.audio(audio_file) # ఇక్కడ క్లోన్ అయిన ఆడియో ప్లే అవుతుంది
            else:
                st.warning("దయచేసి ఏదైనా టెక్స్ట్ టైప్ చేయండి.")
    else:
        st.warning("దయచేసి పైన ఒక ఆడియో ఫైల్ అప్‌లోడ్ చేయండి.")

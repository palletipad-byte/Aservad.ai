...
st.set_page_config(
    page_title="AI Face Swap App", page_icon="🔄", layout="centered"
)

st.title("🔄 AI Face Swap Tool")
st.write(
    "సోర్స్ ఫోటోలోని ముఖాన్ని, టార్గెట్ ఫోటోలోకి మార్చడానికి ఈ యాప్ ఉపయోగపడుతుంది."
)


# 2. Load InsightFace Model (Cached for performance)
@st.cache_resource
def load_model():
...
with st.spinner("AI మోడల్ లోడ్ అవుతోంది, దయచేసి వేచి ఉండండి..."):
  app_model, swapper = load_model()

# 3. Navigation / Choice if you have multiple options
# (ఒకవేళ మీరు మెను లాగా పెట్టి ఉంటే ఇక్కడ ఆప్షన్ తీసుకోవచ్చు)
choice = st.selectbox(
    "మెను ఎంచుకోండి",
    [
        "1. Home",
...
if choice == "5. Face Swap":
  st.subheader("Face Swap సెక్షన్")

  col1, col2 = st.columns(2)

  with col1:
    st.subheader("1. Source Face (ముఖం కావాల్సిన ఫోటో)")
    source_file = st.file_uploader(
        "సోర్స్ ఇమేజ్ అప్లోడ్ చేయండి",
        type=["jpg", "jpeg", "png"],
        key="source",
    )
    if source_file:
  ...
  with col2:
    st.subheader("2. Target Image (ఎక్కడ మార్చాలి)")
    target_file = st.file_uploader(
        "టార్గెట్ ఇమేజ్ అప్లోడ్ చేయండి", type=["jpg", "jpeg", "png"], key="target"
    )
    if target_file:
  ...
  if st.button("🚀 Face Swap ప్రారంభించు", type="primary"):
    if source_file is None or target_file is None:
      st.warning("దయచేసి రెండు ఫోటోలను (Source మరియు Target) అప్లోడ్ చేయండి!")
    else:
      with st.spinner("ఫేస్ స్వాపింగ్ జరుగుతోంది..."):
        try:
          ...
          source_faces = app_model.get(source_cv)
          target_faces = app_model.get(target_cv)

          if len(source_faces) == 0:
            st.error(
                "సోర్స్ ఫోటోలో ఎలాంటి ముఖం గుర్తించబడలేదు. వేరే ఫోటో ట్రై చేయండి."
            )
          elif len(target_faces) == 0:
            st.error(
                "టార్గెట్ ఫోటోలో ఎలాంటి ముఖం గుర్తించబడలేదు. వేరే ఫోటో ట్రై"
                " చేయండి."
            )
          else:
            ...
            st.image(
                final_img, caption="Result Face Swap", use_column_width=True
            )
            ...
            with open("output.jpg", "rb") as file:
              st.download_button(
                  label="📥 ఫైనల్ ఇమేజ్ డౌన్‌లోడ్ చేసుకోండి",
                  data=file,
                  file_name="face_swapped.jpg",
                  mime="image/jpeg",
              )

        except Exception as e:
          st.error(f"ఒక ఎర్రర్ సంభవించింది: {e}")
else:
  st.write(
      "దయచేసి పై మెను నుండి **5. Face Swap** ని ఎంచుకోండి."
    )

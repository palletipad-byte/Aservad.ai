...
# 4. AI ఇమేజ్ జనరేటర్ (Image Generator - Ultra HD & Custom Blend)
elif choice.startswith("4."):
    st.subheader("🎨 AI అడ్వాన్స్‌డ్ ఇమేజ్ జనరేటర్ (Joshna Tailors & Aservad.ai)")
    st.info("మిత్రమా, ఇక్కడ మీరు ఏ రకమైన డిజైన్ లేదా ఫేస్ ఫీచర్స్, స్టైల్స్ మిక్స్ చేసి కావాలన్నా టెక్స్ట్ రూపంలో ఇవ్వండి. అల్ట్రా-హెచ్‌డీ క్వాలిటీలో ఇమేజ్ తయారవుతుంది!")

    # యూజర్ నుండి ప్రాంప్ట్ తీసుకోవడం
    user_img_prompt = st.text_area(
        "మీకు కావలసిన ఇమేజ్ లేదా ఫేస్ కాంబినేషన్ వివరాలు ఇక్కడ రాయండి:", 
        placeholder="ఉదాహరణకు: Blend the sharp eyes, stylish lips, and hair style of famous actors into a cinematic, photorealistic portrait with ultra-HD lighting...",
        key="advanced_img_input"
    )
    
    # స్టైల్ సెలెక్షన్
    img_style = st.selectbox("ఇమేజ్ స్టైల్ ఎంచుకోండి:", [
        "సినిమాటిక్ & ఫోటోరియలిస్టిక్ (Cinematic & Photorealistic)", 
        "ఫేస్ బ్లెండ్ & కస్టమ్ పోర్ట్రైట్ (Face Blend & Custom Portrait)", 
        "ఫ్యాషన్ & బ్లౌజ్ డిజైన్ (Fashion & Designer Look)", 
        "3D యానిమేషన్ ఆర్ట్ (3D Animation Art)"
    ])

    if st.button("🚀 అల్ట్రా-హెచ్‌డీ ఇమేజ్ సృష్టించండి (Generate Image)", key="gen_img_btn"):
        if user_img_prompt.strip():
            with st.spinner("✨ ప్రపంచస్థాయి క్వాలిటీతో ఇమేజ్ రూపుదిద్దుకుంటోంది... దయచేసి వేచి ఉండండి మిత్రమా!"):
                try:
                    client = genai.Client(api_key=st.secrets["GEMINI_API_KEY"])
                    
                    # మరింత పవర్‌ఫుల్ ప్రాంప్ట్ క్రియేషన్
                    enhanced_prompt = f"""
                    Create a hyper-realistic, ultra-HD, cinematic quality image based on the following custom user description and style. 
                    Ensure high details on facial features, lighting, textures, and blending as requested.
                    
                    Style: {img_style}
                    Description/Details: {user_img_prompt}
                    """
                    
                    # ఇమేజ్ జనరేషన్ కోసం జెమినీ మోడల్ కాల్
                    ...
                    for generated_image in response.generated_images:
                        ...
                        st.image(image, caption="Generated by Joshna & Aservad AI", use_column_width=True)
                        
                except Exception as e:
                    st.error(f"⚠️ లోపం ఏర్పడింది: {e}")
        else:
            st.warning("⚠️ దయచేసి ఇమేజ్ గురించిన వివరాలు లేదా ప్రాంప్ట్ రాయండి మిత్రమా!")

...
# 4. AI అల్ట్రా-హెచ్‌డీ ఇమేజ్ జనరేటర్ (Aservad.ai Master Ultra-HD Real Photo Engine)
elif choice.startswith("4."):
    st.subheader("🎨 AI అల్ట్రా-హెచ్‌డీ ఇమేజ్ జనరేటర్ (Joshna Tailors & Aservad.ai)")
    st.info("మిత్రమా, ఇక్కడ మీకు కావలసిన డిజైన్ లేదా ఫోటో వివరాలను రాయండి. ఇది అత్యున్నత నాణ్యతతో అచ్చమైన రియల్ ఫోటోను సృష్టిస్తుంది!")
    ...
    if st.button("🚀 ఇమేజ్ సృష్టించండి (Generate Image)", key="gen_img_btn"):
        if user_img_prompt.strip():
            with st.spinner("⏳ అద్భుతమైన రియల్ ఫోటోను అత్యున్నత నాణ్యతతో సృష్టిస్తున్నాం... దయచేసి వేచి ఉండండి."):
                try:
                    import urllib.parse
                    import requests
                    from PIL import Image
                    ...
                    # 🎯 కార్టూన్ టచ్ పూర్తిగా పోయి, పక్కా రియల్ మనిషి ఫోటో వచ్చేలా పవర్‌ఫుల్ ట్యాగ్స్
                    master_prompt = f"Extreme detailed photography, real human being, photorealistic facial structure, {user_img_prompt}, natural skin texture, detailed pores, shot on 85mm lens, f/1.8, professional studio lighting, 8k resolution, award-winning portrait, absolutely no cartoon, no painting, no illustration."
                    encoded_prompt = urllib.parse.quote(master_prompt)
                    
                    # 📱 మొబైల్ స్క్రీన్ కోసం పర్ఫెక్ట్ 9:16 వర్టికల్ సైజు మరియు హై-క్వాలిటీ మోడల్
                    img_url = f"https://image.pollinations.ai/prompt/{encoded_prompt}?width=768&height=1366&model=flux-realism&nologo=true"
                    
                    response = requests.get(img_url, timeout=60)
                    if response.status_code == 200:
                        image = Image.open(BytesIO(response.content))
                        st.success("✨ అల్ట్రా-హెచ్‌డీ రియల్ ఫోటో విజయవంతంగా తయారైంది మిత్రమా!")
                        st.image(image, caption="Generated by Aservad.ai Master Real Photo Engine", use_container_width=True)
                    else:

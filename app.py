    ...
    if st.button("🚀 ఇమేజ్ సృష్టించండి (Generate Image)", key="gen_img_btn"):
        if user_img_prompt.strip():
            with st.spinner("⏳ అద్భుతమైన డిజైన్‌ను రూపం ఇస్తున్నాం... దయచేసి వేచి ఉండండి."):
                try:
                                        # డైరెక్ట్ ఉచిత Hugging Face API ఎండ్-పాయింట్
                    ...
                    if response.status_code == 200:
                        ...
                        st.image(image, caption="Generated by Aservad.ai & Hugging Face (Free)")
                    else:
                        st.error(f"⚠️ లోపం ఏర్పడింది: సర్వర్ బిజీగా ఉంది లేదా రెస్పాన్స్ కోడ్ {response.status_code}")
                        
                    
                    st.success("✨ ఇమేజ్ విజయవంతంగా తయారైంది మిత్రమా!")
                    st.image(image, caption="Generated by Aservad.ai & Hugging Face")

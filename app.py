...
# 4. AI అల్ట్రా-హెచ్‌డీ ఇమేజ్ జనరేటర్ (Aservad.ai Ultra-HD FLUX Engine)
elif choice.startswith("4."):
    st.subheader("🎨 AI అల్ట్రా-హెచ్‌డీ ఇమేజ్ జనరేటర్ (Joshna Tailors & Aservad.ai)")
    st.info("మిత్రమా, ఇక్కడ మీకు కావలసిన డిజైన్ లేదా ఫోటో వివరాలను రాయండి. మీరు ఏది రాస్తే దానిని ఇది అద్భుతమైన రియల్ ఫోటోగా సృష్టిస్తుంది!")
    
    # యూజర్ ఇచ్చే ఇన్‌పుట్ (ఇక్కడ ఎవరి పేరు లేదా డిజైన్ ఫిక్స్ చేసి ఉండదు, యూజర్ ఇష్టమే సర్వస్వం!)
    user_img_prompt = st.text_area(
        "మీకు కావలసిన డిజైన్ లేదా ఇమేజ్ వివరాలు ఇక్కడ రాయండి:",
        placeholder="ఉదాహరణకు: A beautiful traditional designer silk saree with intricate gold jewelry, professional lighting..."
    )
    
    if st.button("🚀 ఇమేజ్ సృష్టించండి (Generate Image)", key="gen_img_btn"):
        if user_img_prompt.strip():
            with st.spinner("⏳ మీరు కోరిన డిజైన్‌ను అత్యున్నత నాణ్యతతో రూపం ఇస్తున్నాం... దయచేసి వేచి ఉండండి."):
                try:
                    import urllib.parse
                    import requests
                    from PIL import Image
                    ...
                    # 🎯 యూజర్ ఇచ్చిన ప్రాంప్ట్‌ని మాత్రమే ప్రధానంగా తీసుకుని, కార్టూన్ రాకుండా క్వాలిటీ ట్యాగ్స్ జత చేస్తున్నాం
                    base_prompt = f"{user_img_prompt}. [IMPORTANT: STRICTLY A PHOTOREALISTIC REAL-LIFE PHOTO, NOT AN ILLUSTRATION, NOT A DRAWING, NOT A PAINTING]. Cinematic lighting, highly detailed features, realistic skin texture, 8k UHD, professional studio photography, masterpiece."
                    encoded_prompt = urllib.parse.quote(base_prompt)
                    
                    # 📱 మొబైల్ స్క్రీన్‌కి సరిపడా వర్టికల్ సైజు (9:16 Aspect Ratio)
                    img_url = f"https://image.pollinations.ai/prompt/{encoded_prompt}?width=720&height=1280&model=flux&nologo=true"

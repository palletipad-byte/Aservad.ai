...
# 4. AI అడ్వాన్స్‌డ్ ఇమేజ్ జనరేటర్ (Google Imagen API - MASTER Version)
elif choice.startswith("4."):
    st.subheader("🎨 AI అల్ట్రా-హెచ్‌డీ ఇమేజ్ జనరేటర్ (Joshna Tailors & Aservad.ai)")
    st.info("మిత్రమా, ఇక్కడ మీకు కావలసిన డిజైన్ లేదా ఫోటో వివరాలను రాయండి. ఇది నేరుగా గూగుల్ ఏఐ సహాయంతో రియలిస్టిక్ ఫోటోను సృష్టిస్తుంది!")
    ...
    if st.button("🚀 ఇమేజ్ సృష్టించండి (Generate Image)", key="gen_img_btn"):
        if user_img_prompt.strip():
            with st.spinner("⏳ గూగుల్ ఏఐ సహాయంతో అద్భుతమైన ఫోటోను సృష్టిస్తున్నాం... దయచేసి వేచి ఉండండి."):
                try:
                    from google import genai
                    from PIL import Image
                    from io import BytesIO
                    
                    # 🎯 నేరుగా గూగుల్ జెమినీ ఏఐ (Imagen) మోడల్‌ని వాడటం
                    client = genai.Client(api_key=st.secrets["GEMINI_API_KEY"])
                    
                    result = client.models.generate_images(
                        model='imagen-3.0-generate-002',
                        prompt=f"{user_img_prompt}, photorealistic, real human face, 8k resolution, highly detailed skin texture",
                        config=dict(
                            number_of_images=1,
                            output_mime_type="image/jpeg",
                            aspect_ratio="3:4",  # మొబైల్ పోర్ట్రేట్ సైజు
                    ...
                    for generated_image in result.generated_images:
                        ...
                        st.image(image, caption="Generated by Google Imagen AI & Aservad.ai", use_container_width=True)

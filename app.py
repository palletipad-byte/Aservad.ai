...
# 4. AI అడ్వాన్స్‌డ్ ఇమేజ్ జనరేటర్ (Aservad.ai Ultra-HD FLUX Engine - PRO Version)
elif choice.startswith("4."):
    st.subheader("🎨 AI అల్ట్రా-హెచ్‌డీ ఇమేజ్ జనరేటర్ (Joshna Tailors & Aservad.ai)")
    st.info("మిత్రమా, ఇక్కడ మీకు కావలసిన డిజైన్ లేదా ఫోటో వివరాలను రాయండి. ఇది అత్యంత అద్భుతమైన రియలిస్టిక్ ఫోటోను సృష్టిస్తుంది!")
    
    user_img_prompt = st.text_area(
        "మీకు కావలసిన డిజైన్ లేదా ఇమేజ్ వివరాలు ఇక్కడ రాయండి:",
        placeholder="ఉదాహరణకు: A highly detailed photorealistic portrait of Nayanthara in a saree..."
    )
        ...
        if user_img_prompt.strip():
            with st.spinner("⏳ అద్భుతమైన డిజైన్‌ను రూపం ఇస్తున్నాం... దయచేసి వేచి ఉండండి."):
                try:
                    ...
                    # 🎯 పవర్‌ఫుల్ ప్రాంప్ట్ ఆప్టిమైజేషన్ (యానిమేషన్ రాకుండా, రియలిస్టిక్ ఫోటో కోసం)
                    # మనం యూజర్ ప్రాంప్ట్‌కి "{user_img_prompt}. [MUST BE A HIGHLY DETAILED PHOTOREALISTIC PHOTO, NOT ART OR CARTOON]. Studio quality, 8k resolution, ultra-realistic, cinematic lighting" అని జత చేస్తున్నాం.
                    
                    full_prompt = f"{user_img_prompt}. [MUST BE A HIGHLY DETAILED PHOTOREALISTIC PHOTO, NOT ART OR CARTOON, NOT ILLUSTRATION]. Studio quality, 8k resolution, ultra-realistic, cinematic lighting, realistic skin texture, shot on 85mm lens."
                    encoded_prompt = urllib.parse.quote(full_prompt)
                    
                    # 📱 మొబైల్ స్క్రీన్‌కి సరిపడా వర్టికల్ సైజు (Portrait Mode)
                    # width=1024&height=1536 (ఇది నిలువుగా ఎక్కువ స్పేస్ తీసుకుంటుంది)
                    img_url = f"https://image.pollinations.ai/prompt/{encoded_prompt}?width=1024&height=1536&model=flux&nologo=true"
                    
                    response = requests.get(img_url, timeout=60)
                    if response.status_code == 200:
                        image = Image.open(BytesIO(response.content))
                        st.success("✨ ఇమేజ్ విజయవంతంగా తయారైంది మిత్రమా!")
                        # st.image లో use_column_width=True ఇవ్వడం వల్ల ఫోటో వెబ్ పేజీ వెడల్పు ఎంత ఉంటే అంత పడుతుంది.
                        st.image(image, caption="Generated by Aservad.ai Ultra-HD Engine", use_column_width=True)
                    else:

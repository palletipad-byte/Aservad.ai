...
# 4. AI అడ్వాన్స్‌డ్ ఇమేజ్ జనరేటర్ (Aservad.ai Ultra-HD FLUX Engine - MAX PRO Version)
elif choice.startswith("4."):
    st.subheader("🎨 AI అల్ట్రా-హెచ్‌డీ ఇమేజ్ జనరేటర్ (Joshna Tailors & Aservad.ai)")
    st.info("మిత్రమా, ఇక్కడ మీకు కావలసిన డిజైన్ లేదా ఫోటో వివరాలను రాయండి. ఇది అత్యున్నత నాణ్యతతో కూడిన రియలిస్టిక్ ఫోటోను సృష్టిస్తుంది!")
    
    user_img_prompt = st.text_area(
        "మీకు కావలసిన డిజైన్ లేదా ఇమేజ్ వివరాలు ఇక్కడ రాయండి:",
        placeholder="ఉదాహరణకు: A highly detailed photograph of a real human face, Nayanthara smiling in a silk saree..."
    )
    
    if st.button("🚀 ఇమేజ్ సృష్టించండి (Generate Image)", key="gen_img_btn"):
        if user_img_prompt.strip():
            with st.spinner("⏳ అద్భుతమైన డిజైన్‌ను అత్యున్నత నాణ్యతతో రూపం ఇస్తున్నాం... దయచేసి వేచి ఉండండి."):
                try:
                    import urllib.parse
                    import requests
                    from PIL import Image
                    ...
                    # 🎯 గూగుల్ ఏఐ లెవెల్ పర్ఫెక్ట్ రియలిస్టిక్ ప్రాంప్ట్ ఆప్టిమైజేషన్
                    # మనం యూజర్ ప్రాంప్ట్‌కి ముందు 'A highly detailed photograph of a real human face' అని గట్టిగా జత చేస్తున్నాం.
                    # అలాగే 'not a drawing, not animation, not cartoon' అని నెగెటివ్ ప్రాంప్ట్ కూడా ఇస్తున్నాం.
                    base_prompt = f"A highly detailed photograph of a real human face, {user_img_prompt}. [IMPORTANT: STRICTLY A REAL-LIFE PHOTO, NOT DRAWING, NOT ILLUSTRATION, NOT CARTOON]. Realistic skin texture, high detail, 8k resolution, cinematic lighting, photorealistic masterpiece."
                    encoded_prompt = urllib.parse.quote(base_prompt)
                    
                    # 📱 మొబైల్ స్క్రీన్‌కి సరిపడా వర్టికల్ సైజు మరియు ఫాస్టెస్ట్ మోడల్
                    img_url = f"https://image.pollinations.ai/prompt/{encoded_prompt}?width=768&height=1024&model=flux&nologo=true&seed=42"
                    
                    response = requests.get(img_url, timeout=60)
                    if response.status_code == 200:
                        image = Image.open(BytesIO(response.content))
                        st.success("✨ ఇమేజ్ విజయవంతంగా తయారైంది మిత్రమా!")
                        # st.image లో use_container_width=True ఇవ్వడం వల్ల ఫోటో వెబ్ పేజీ వెడల్పు ఎంత ఉంటే అంత పడుతుంది.
                        st.image(image, caption="Generated by Aservad.ai Ultra-HD Engine", use_container_width=True)
                    else:
                        st.error("⚠️ సర్వర్ స్పందించడం లేదు, దయచేసి ఒకసారి మళ్లీ ప్రయత్నించండి మిత్రమా!")
                except Exception as e:
                    st.error(f"⚠️ లోపం ఏర్పడింది: {e}")
        else:

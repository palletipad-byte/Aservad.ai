...
# 4. AI అల్ట్రా-హెచ్‌డీ ఇమేజ్ జనరేటర్ (Aservad.ai Real Photography Engine)
elif choice.startswith("4."):
    st.subheader("🎨 AI అల్ట్రా-హెచ్‌డీ ఇమేజ్ జనరేటర్ (Joshna Tailors & Aservad.ai)")
    st.info("మిత్రమా, ఇక్కడ మీకు కావలసిన డిజైన్ లేదా ఫోటో వివరాలను రాయండి. ఇది అచ్చమైన రియల్ ఫోటోను సృష్టిస్తుంది!")
    
    user_img_prompt = st.text_area(
        "మీకు కావలసిన డిజైన్ లేదా ఇమేజ్ వివరాలు ఇక్కడ రాయండి:",
        placeholder="ఉదాహరణకు: A real human portrait of Nayanthara in a silk saree..."
    )
    
    if st.button("🚀 ఇమేజ్ సృష్టించండి (Generate Image)", key="gen_img_btn"):
        if user_img_prompt.strip():
            with st.spinner("⏳ అసలైన రియల్ ఫోటోను కెమెరా క్వాలిటీతో సృష్టిస్తున్నాం... దయచేసి వేచి ఉండండి."):
                try:
                    import urllib.parse
                    import requests
                    from PIL import Image
                    ...
                    # 🎯 కార్టూన్ లేదా పెయింటింగ్ లుక్ పూర్తిగా రాకుండా స్ట్రాంగ్ రియల్ కెమెరా కమాండ్స్
                    base_prompt = f"Raw candid photography, real human being, natural skin texture, skin pores, shot on 35mm lens, DSLR, photorealistic, {user_img_prompt}, no illustration, no painting, no 3d render, 8k."
                    encoded_prompt = urllib.parse.quote(base_prompt)
                    
                    # 📱 'flux-realism' మోడల్ వాడితే కార్టూన్ లుక్ రాకుండా అచ్చమైన రియల్ ఫోటో వస్తుంది
                    img_url = f"https://image.pollinations.ai/prompt/{encoded_prompt}?width=720&height=1280&model=flux-realism&nologo=true"
                    
                    response = requests.get(img_url, timeout=60)
                    if response.status_code == 200:
                        image = Image.open(BytesIO(response.content))
                        st.success("✨ అసలైన రియల్ ఫోటో విజయవంతంగా తయారైంది మిత్రమా!")
                        st.image(image, caption="Generated by Aservad.ai Real Photo Engine", use_container_width=True)
                    else:

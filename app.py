    ...
    if st.button("🚀 వాయిస్ జనరేట్ చేయి", type="primary"):
        if not api_key or not voice_id:
            ...
            with st.spinner("⏳ వాయిస్ తయారవుతోంది, దయచేసి వేచి ఉండండి..."):
                url = f"https://api.elevenlabs.io/v1/text-to-speech/NOpBlnGInO9m6vDvFkFC"
                headers = {
                    "Accept": "audio/mpeg",
...
# 4. AI ఇమేజ్ జనరేటర్ (Smart Auto-Enhance Image Generator)
    ...
    st.info("మిత్రమా, ఇక్కడ మీరు ఏ డిజైన్, బ్లౌజ్ మోడల్ లేదా ఫోటో కావాలన్నా సింపుల్‌గా తెలుగు లేదా ఇంగ్లీష్‌లో అడగండి. ఏఐ దాన్ని ఆటోమేటిక్‌గా అల్ట్రా-హెచ్‌డీ క్వాలిటీలో సృష్టిస్తుంది!")

    user_img_prompt = st.text_area(
        "మీకు కావలసిన డిజైన్ లేదా ఇమేజ్ వివరాలు ఇక్కడ రాయండి (ఉదా: బ్లౌజ్ డిజైన్, ఫ్యాషన్ లుక్, మొదలైనవి):", 
        placeholder="ఉదాహరణకు: ఎంబ్రాయిడరీ వర్క్‌తో కూడిన అందమైన మోడ్రన్ బ్లౌజ్ డిజైన్...",
        key="advanced_img_input"
    )

    if st.button("🚀 ఇమేజ్ సృష్టించండి (Generate Image)", key="gen_img_btn"):
        if user_img_prompt.strip():
            with st.spinner("✨ మీ ఆలోచనకు రూపం ఇస్తున్నాం... దయచేసి వేచి ఉండండి మిత్రమా!"):
                try:
                    client = genai.Client(api_key=st.secrets["GEMINI_API_KEY"])
                    
                    # యూజర్ ఏది అడిగినా దాన్ని ఆటోమేటిక్‌గా అల్ట్రా-హెచ్‌డీ క్వాలిటీలోకి మార్చే స్మార్ట్ ప్రాంప్ట్
                    smart_prompt = f"""
                    Generate a hyper-realistic, ultra-HD, high-detail professional image based on the following user request. 
                    Enhance it automatically with stunning lighting, clarity, and professional styling, whether it's a fashion design, blouse model, or portrait:
                    
                    User Request: {user_img_prompt}
                    """
                    ...
                    if not image_found:
                        st.warning("⚠️ ఇమేజ్ జనరేట్ కాలేదు, దయచేసి మరోసారి ప్రయత్నించండి.")
                        
                except Exception as e:
                    st.error(f"⚠️ లోపం ఏర్పడింది: {e}")
        else:
            st.warning("⚠️ దయచేసి మీకు కావలసిన డిజైన్ లేదా వివరాలు రాయండి మిత్రమా!")
